...
export async function up({ context: sequelize }: MigrationParams<Sequelize>): Promise<void> {
  ...
  await qi.addIndex('restaurant_tables', ['tenantId']);
  await qi.addIndex('restaurant_tables', ['sectionId']);
  await qi.addIndex('restaurant_tables', ['status']);

  await sequelize.query(
    'CREATE UNIQUE INDEX IF NOT EXISTS "restaurant_tables_number_section_unique" ON "restaurant_tables" ("sectionId", "number") WHERE "deletedAt" IS NULL',
  );
}
