...
export async function up({ context: sequelize }: MigrationParams<Sequelize>): Promise<void> {
  ...
  await qi.createTable('pos_sessions', {
    ...
    terminalId: { type: DataTypes.UUID, allowNull: false },
    status: { type: DataTypes.STRING(20), allowNull: false, defaultValue: 'open' },
    openingFloat: { type: DataTypes.DECIMAL(15, 2), allowNull: false, defaultValue: 0 },
  ...
  await qi.addIndex('pos_sessions', ['tenantId', 'cashierId', 'status']);
  await qi.addIndex('pos_sessions', ['branchId']);
}
