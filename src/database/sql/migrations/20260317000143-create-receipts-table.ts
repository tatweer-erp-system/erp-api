...
export async function up({ context: sequelize }: MigrationParams<Sequelize>): Promise<void> {
  ...
  await qi.addIndex('receipts', ['tenantId']);
  await qi.addIndex('receipts', ['tenantId', 'branchId']);
  await qi.addIndex('receipts', ['tenantId', 'purchaseOrderId']);
  await qi.addIndex('receipts', ['tenantId', 'status']);
  await qi.addIndex('receipts', ['tenantId', 'reference'], { unique: true });
}

export async function down({ context: sequelize }: MigrationParams<Sequelize>): Promise<void> {
  await sequelize.getQueryInterface().dropTable('receipts');
}
