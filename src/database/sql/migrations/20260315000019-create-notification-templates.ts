...
export async function up({ context: sequelize }: MigrationParams<Sequelize>): Promise<void> {
  ...
  await qi.createTable('notification_templates', {
    ...
    subjectEn: { type: DataTypes.STRING(255), allowNull: true },
    subjectAr: { type: DataTypes.STRING(255), allowNull: true },
  ...
  await qi.addIndex('notification_templates', ['tenantId', 'eventType', 'channel']);
}

export async function down({ context: sequelize }: MigrationParams<Sequelize>): Promise<void> {
  await sequelize.getQueryInterface().dropTable('notification_templates');
}
