...
export class ProjectsService {
  ...
  async findAll(tenantSlug: string, pagination: PaginationDto) {
    ...
    const [rows] = await sequelize.query(
      `SELECT * FROM projects WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT :limit OFFSET :offset`,
  ...
  async remove(tenantSlug: string, id: string): Promise<void> {
    ...
    const sequelize = await this.tenantSequelizeService.getSequelizeForTenant(tenantSlug);
    await sequelize.query(`UPDATE projects SET deleted_at = NOW() WHERE id = :id`, {
      replacements: { id },
    } as any);
  }
}
