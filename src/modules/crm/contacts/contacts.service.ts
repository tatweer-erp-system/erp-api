...
export class ContactsService {
  ...
  async findAll(tenantSlug: string, pagination: PaginationDto) {
    ...
    const whereClause = search ? `AND (first_name ILIKE :search OR last_name ILIKE :search OR email ILIKE :search OR company ILIKE :search)` : '';
  ...
  async findOne(tenantSlug: string, id: string) {
    const sequelize = await this.tenantSequelizeService.getSequelizeForTenant(tenantSlug);
    const [rows] = await sequelize.query(`SELECT * FROM contacts WHERE id = :id AND deleted_at IS NULL`, { replacements: { id }, type: 'SELECT' } as any);
  ...
  async create(tenantSlug: string, dto: CreateContactDto, createdBy?: string) {
    ...
    await sequelize.query(
      ...
      { replacements: { id, ...dto, email: dto.email ?? null, phone: dto.phone ?? null, company: dto.company ?? null, position: dto.position ?? null, notes: dto.notes ?? null, assignedTo: dto.assignedTo ?? null, createdBy: createdBy ?? null } } as any,
    );
  ...
  async remove(tenantSlug: string, id: string): Promise<void> {
    ...
    const sequelize = await this.tenantSequelizeService.getSequelizeForTenant(tenantSlug);
    await sequelize.query(`UPDATE contacts SET deleted_at = NOW() WHERE id = :id`, { replacements: { id } } as any);
  }
}
