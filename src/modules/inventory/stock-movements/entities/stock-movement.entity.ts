...
export class StockMovement extends Model {
  ...
  @Column({ type: DataType.STRING(50), allowNull: false, field: 'movement_type' }) movementType!: string;
  @Column({ type: DataType.DECIMAL(12, 3), allowNull: false }) quantity!: number;
  @Column({ type: DataType.DECIMAL(12, 3), allowNull: false, field: 'quantity_before' }) quantityBefore!: number;
  @Column({ type: DataType.DECIMAL(12, 3), allowNull: false, field: 'quantity_after' }) quantityAfter!: number;
  @Column({ type: DataType.TEXT, allowNull: true }) notes!: string | null;
  @Column({ type: DataType.UUID, allowNull: true, field: 'reference_id' }) referenceId!: string | null;
  @Column({ type: DataType.STRING(50), allowNull: true, field: 'reference_type' }) referenceType!: string | null;
