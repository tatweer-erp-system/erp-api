...
export class PurchaseOrdersService {
  ...
  async findOne(tenantSlug: string, id: string) {
    const sequelize = await this.tenantSequelizeService.getSequelizeForTenant(tenantSlug);
    const [rows] = await sequelize.query(`SELECT * FROM purchase_orders WHERE id = :id AND deleted_at IS NULL`, { replacements: { id }, type: 'SELECT' } as any);
    const order = (rows as any[])[0];
    if (!order) throw new NotFoundException('Purchase order not found');
    const [lines] = await sequelize.query(`SELECT * FROM purchase_order_lines WHERE order_id = :id`, { replacements: { id }, type: 'SELECT' } as any);
    return { ...order, lines };
  }
  ...
  async create(tenantSlug: string, dto: CreatePurchaseOrderDto, createdBy?: string) {
    ...
    let subtotal = 0;
    for (const line of dto.lines) { subtotal += line.quantity * line.unitPrice; }
    await sequelize.query(
      ...
      { replacements: { id, orderNumber, vendorId: dto.vendorId ?? null, subtotal, currency: dto.currency ?? 'USD', expectedDeliveryDate: dto.expectedDeliveryDate ?? null, notes: dto.notes ?? null, createdBy: createdBy ?? null } } as any,
    );
    for (const line of dto.lines) {
      await sequelize.query(
        ...
        { replacements: { id: uuidv4(), orderId: id, productId: line.productId ?? null, description: line.description, quantity: line.quantity, unitPrice: line.unitPrice, lineTotal: line.quantity * line.unitPrice } } as any,
      );
    }
